...
- Stack in one line: OpenAPI 3.1 YAML, JSON Schema (events), Gherkin (`.feature`), Markdown ADRs,
  versioned prompt files, PromptFoo evals. No application code — this repo IS the intent that other
  repos' PRs get reviewed against, which makes `--dry-run` mode the primary mode here (see
  `SKILL.md`): most `motifpath-specs` changes precede code, they don't follow it.
...
| Every event requires `event_type`, `student_id`, `session_id`, `occurred_at`; no optional field added without a Gherkin scenario exercising it | `motifpath-specs/CLAUDE.md` — Event Schema Standards |
...
- **Tolerated legacy:** none documented yet — ask the user before treating anything as
  grandfathered.
- **Automatic gates (don't re-flag):** Redocly CLI lint on `openapi/*.yaml`, `ajv` validation on
  `events/*.json`, Gherkin syntax parse on `features/*.feature`, PromptFoo eval on `prompts/` changes
  — all run in `.github/workflows/ci.yml`.
